...
function parseMarkdown(md) {
  ...
  // Render Headings
  html = html.replace(/^### (.*$)/gim, '<h4 class="font-bold text-white text-base mt-4 mb-2">$1</h4>');
  html = html.replace(/^## (.*$)/gim, '<h3 class="font-bold text-white text-lg mt-5 mb-2">$1</h3>');
  html = html.replace(/^# (.*$)/gim, '<h2 class="font-bold text-white text-xl mt-6 mb-3">$1</h2>');
  
  // Render Bold: **text**
  html = html.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
  
  // Render Inline Code: `code`
  html = html.replace(/`(.*?)`/g, '<code class="bg-white/10 px-1.5 py-0.5 rounded text-xs text-secondary font-mono">$1</code>');

  // Render Links: [text](url)
  html = html.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" target="_blank" class="text-secondary hover:underline">$1</a>');
  ...
  for (let line of lines) {
    ...
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      if (!inList) {
        result.push('<ul class="list-disc pl-5 my-2 space-y-1 text-slate-300">');
        inList = true;
      }
...
function renderRelease(data) {
  ...
  const mainBtnLabel = mainAsset === winPortable 
  ...
  setTimeout(() => {
    contentEl.querySelectorAll('.reveal').forEach(el => el.classList.add('active'));
  }, 50);
}
